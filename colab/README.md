...
### 02_streaming_chat_loop.ipynb

Mirrors `02_streaming_chat_loop.py`. Because Colab does not have an interactive `input()` loop in the traditional sense, this notebook shows a fixed list of questions and streams the answers one by one, printing tokens as they arrive. You can edit the question list in the cell to try your own prompts.

### 03_system_prompt.ipynb

Mirrors `03_system_prompt.py`. Shows how to set a system persona before the first user message. The notebook uses the same concise Python expert persona from the script and asks two follow-up questions to demonstrate that the persona carries across turns.

### 04_gpu_backend.ipynb
...
Mirrors `05_speculative_decoding.py`. Adds multi-token prediction on top of the GPU backend. The notebook includes a timing cell before and after so you can see the difference in tokens per second. Requires a GPU runtime.

### 06_tool_use.ipynb

Mirrors `06_tool_use.py`. Defines Python functions as tools and passes them to the model. The notebook includes an extra section that shows the raw tool-call event from the model before the result is injected, so you can see exactly what the model emits when it decides to call a function.

### 07_multimodal_audio.ipynb

Mirrors `07_multimodal_audio.py`. Includes a cell that generates a short test `.wav` file using the standard library so you do not need to upload your own audio. Swap the generated file for a real recording if you want to test transcription or summarisation.
...
### 09_streaming_with_system_prompt.ipynb
...
4. Runs the same four OpenAI API patterns and four Gemini API patterns from the Python script. Each pattern is its own cell so you can run just the one you are interested in.
